...
public class SlowConvexHull {
	...
	private ArrayList<Point> sortedVertexList(ArrayList<LineSegment> lines) {
		final ArrayList<LineSegment> xSorted = (ArrayList<LineSegment>) lines.clone();
		Collections.sort(xSorted, new XCompare());

		final int n = xSorted.size();

		final LineSegment baseLine = new LineSegment(xSorted.get(0).p1, xSorted.get(n - 1).p1);
	...
	public ArrayList<Point> execute(ArrayList<Point> points) {
		...
		final ArrayList<LineSegment> edges = new ArrayList<LineSegment>();
