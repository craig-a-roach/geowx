/*
 * Copyright 2012 Meteorological Service of New Zealand Limited all rights reserved. No part of this work may be stored
 * in a retrievable system, transmitted or reproduced in any way without the prior written permission of the
 * Meteorological Service of New Zealand
 */
package com.metservice.beryllium.mirror;

import com.metservice.argon.IArgonFileProbe;
import com.metservice.argon.IArgonRunProbe;
import com.metservice.beryllium.IBerylliumNetProbe;

/**
 * @author roach
 */
public interface IBerylliumMirrorProbe extends IBerylliumNetProbe, IArgonFileProbe, IArgonRunProbe {

	boolean isLiveMirror();

	void liveMirror(String message, Object... args);

	void warnMirror(String diagnostic);
}
