...
abstract class AbstractProjectionAzimuthal extends AbstractProjection {
	...
	public void initialize()
		...
		if (Math.abs(Math.abs(projectionLatitude) - MapMath.HALFPI) < EPS10) {
			mode = projectionLatitude < 0.0 ? SOUTH_POLE : NORTH_POLE;
	...
	public boolean inside(double lon, double lat) {
		return MapMath.greatCircleDistance(Math.toRadians(lon), Math.toRadians(lat), projectionLongitude, projectionLatitude) < Math
				.toRadians(mapRadius);
	}
