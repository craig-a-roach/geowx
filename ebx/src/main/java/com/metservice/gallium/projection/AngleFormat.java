 ...
 * @author roach
 */
class AngleFormat extends NumberFormat {
	...
	public StringBuffer format(double number, StringBuffer result, FieldPosition fieldPosition) {
		...
		final double ddmmss = isDegrees ? number : Math.toDegrees(number);
		int iddmmss = (int) Math.round(ddmmss * 3600);
		if (iddmmss < 0) {
			iddmmss = -iddmmss;
		}
		final int fraction = iddmmss % 3600;
	...
	public Number parse(String text, ParsePosition parsePosition) {
		...
		if (length > 0) {
			final char c = Character.toUpperCase(text.charAt(length - 1));
			switch (c) {
				case 'W':
				case 'S':
					negate = true;
		...
		if (i != -1) {
			final String dd = text.substring(0, i);
			...
			if (i != -1) {
				if (i != 0) {
					final String mm = mmss.substring(0, i);
				...
				if (i != mmss.length() - 1) {
					final String ss = mmss.substring(i + 1);
					s = Double.valueOf(ss).doubleValue();
				}
				if (m < 0 || m > 59) throw new NumberFormatException("Minutes must be between 0 and 59");
				if (s < 0 || s >= 60) throw new NumberFormatException("Seconds must be between 0 and 59");
			} else if (i != 0) {
