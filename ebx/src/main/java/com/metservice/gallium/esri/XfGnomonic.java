 ...
 * @author roach
 */
class XfGnomonic extends AbstractProjectionFactory {
	...
	public IGalliumProjection newProjection(ParameterMap pmap, GeographicCoordinateSystem gcs, Unit pu)
		...
		if (Math.abs(aphi0 - MapMath.HALFPI) < MapMath.EPS10) {
			...
			arg = new XaGnomonicPolar(north);
		} else {
			if (aphi0 < MapMath.EPS10) {
				arg = new XaGnomonicEquator();
			} else {
				arg = new XaGnomonicOblique(phi0);
			}
		}
		return new XpGnomonic(oAuthority, title, argBase, arg);
	}
