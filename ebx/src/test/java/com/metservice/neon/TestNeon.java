...
public abstract class TestNeon {
	...
	public void setupSpace() {
		...
		try {
			...
			space.start();
			if (spaceCfg.getAutoDebugPattern() != null) {
				System.out.println("Debug on port " + spaceId.listenPort() + " (make sure scripts are in NeonIndex.txt)");
			}
			c_space = space;
	...
	protected static class ExpectedValue {
		...
		public String mismatch(EsExecutionContext ecx, IEsOperand actual)
			...
			switch (target) {
				...
				break;
				case '?':
					oCanonActual = Boolean.valueOf(actual.toCanonicalBoolean());
				break;
				case '#':
					oCanonActual = new Integer(actual.toNumber(ecx).intVerified());
				break;
				case '^':
					oCanonActual = actual.toNumber(ecx).realValue();
				break;
				case '%':
					oCanonActual = new Double(actual.toNumber(ecx).doubleValue());
				break;
				case '&':
					oCanonActual = new Long(actual.toNumber(ecx).longValue());
				break;
				case '@':
