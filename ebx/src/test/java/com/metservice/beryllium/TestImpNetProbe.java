 ...
 * @author roach
 */
public class TestImpNetProbe implements IBerylliumNetProbe {
	...
	public void failNet(Ds diagnostic) {
		if (m_sysout) {
			System.out.println(qts() + " failNet[" + diagnostic.ss() + "]");
		}
	...
	public void liveNet(String message, Object... args) {
		if (isLiveNet()) {
			...
			System.out.println(qts() + " liveNet[" + sb + "]");
		}
	}
	...
	public void warnNet(Ds diagnostic) {
		if (m_sysout) {
			System.out.println(qts() + " warnNet[" + diagnostic.ss() + "]");
		}
		addWarn("Net", diagnostic.s());
	}

	public TestImpNetProbe(boolean sysout, boolean showLive) {
		m_sysout = sysout;
		m_showLive = showLive;
